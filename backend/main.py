...
class GestureDetectionSystem:
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Detect gestures every few frames
            if int(time.time()) % 2 == 0:  # Every 2 seconds
                predictions = self.detect_gestures(frame)
                if predictions:
                    self.handle_play_pause_toggle(predictions)
...
def enroll():
    """Enroll a new user with face embeddings"""
    try:
        body = request.get_json()
        email = body.get("email")
        name = body.get("name")
        frames = body.get("frames", [])  # List of base64 data URLs
        
        # Validate input
        if not email or not frames:
        ...
        if users_collection:
            user = users_collection.find_one_and_update(
                {"email": email},
                {"$setOnInsert": {"email": email, "name": name}, "$set": {"active": True}},
        ...
        for i, frame in enumerate(frames):
            ...
            embeddings = get_face_embeddings_hybrid_improved(frame)
            
            if not embeddings:
                print(f"No embeddings extracted from frame {i+1}")
                continue
            
            # Take the first face embedding per frame
            ...
            payload = {
                ...
                "email": email,
                "name": name,
                "version": "v2",  # Updated version for improved embeddings
                "frameIndex": i,
                "timestamp": time.time()
            }
        ...
        if points and qdrant:
            ...
            print(f"✅ Inserted {len(points)} face embeddings to Qdrant")
            
            # Update MongoDB with Qdrant point IDs
            if users_collection:
                users_collection.update_one(
                    {"_id": ObjectId(user_id)},
                    {"$push": {"qdrant_ids": {"$each": qdrant_ids}}}
                )
        ...
        if points:
            ...
            return jsonify({"error": "no_embeddings_extracted"}), 500
...
def face_auth():
    ...
    try:
        ...
        if not embeddings:
            print("❌ No embeddings generated from image")
            return jsonify({"matched": False, "reason": "no_face_detected"}), 200
        
        print(f"✅ Generated {len(embeddings)} embeddings from image")
        
        # Use the first embedding
        query_embedding = normalize_vector(embeddings[0])
        print(f"📊 Query embedding length: {len(query_embedding)}")
        
        # Query Qdrant for top 5 nearest neighbors
        if not qdrant:
            print("❌ Qdrant not available")
            return jsonify({"matched": False, "reason": "vector_db_unavailable"}), 500
        
        print("🔍 Searching Qdrant for similar faces...")
        hits = qdrant.search(
            collection_name=COLLECTION_NAME,
        ...
        print(f"🔍 Qdrant search returned {len(hits)} hits")
        
        if not hits:
            print("❌ No matches found in Qdrant")
            return jsonify({"matched": False, "reason": "no_matches"}), 200
        
        # Log all hits for debugging
        for i, hit in enumerate(hits):
            print(f"  Hit {i+1}: score={hit.score:.4f}, userId={hit.payload.get('userId')}, name={hit.payload.get('name')}")
        
        # Aggregate scores by userId
        scores_by_user = {}
        for hit in hits:
            user_id = hit.payload.get("userId")
            if user_id:
                scores_by_user.setdefault(user_id, []).append(hit.score)
        
        print(f"📊 Scores by user: {scores_by_user}")
        
        # Find the user with the best score
        best_user_id = None
        best_score = -1.0
        ...
        print(f"🏆 Best match: userId={best_user_id}, score={best_score:.4f}, threshold={THRESHOLD}")
        
        # Apply threshold
        if best_user_id and best_score >= THRESHOLD:
            print(f"✅ Authentication successful! Score {best_score:.4f} >= threshold {THRESHOLD}")
            
            # Get user details from Qdrant payload (since MongoDB might not be available)
            user_info = None
            
            # Find the best hit to get user details from payload
            best_hit = None
            for hit in hits:
                if hit.payload.get("userId") == best_user_id:
                    best_hit = hit
                    break
            
            if best_hit and best_hit.payload:
                # Use data from Qdrant payload
                user_info = {
                    "id": best_user_id,
                    "email": best_hit.payload.get("email", "unknown"),
                    "name": best_hit.payload.get("name", "Unknown User")
                }
                print(f"👤 User info from Qdrant payload: {user_info}")
            elif users_collection:
                # Fallback to MongoDB if available
                user = users_collection.find_one({"_id": ObjectId(best_user_id)})
                if user:
                    user_info = {
                        "id": best_user_id,
            ...
            if not user_info:
                # Final fallback if neither Qdrant payload nor MongoDB has the data
                user_info = {
                    "id": best_user_id,
                    "email": "unknown",
                    "name": "Unknown User"
                }
                print(f"⚠️ Using fallback user info: {user_info}")
...
def gesture_status():
    ...
    return jsonify({
        'success': True,
        'running': gesture_system.running
    })
...
def play_specific_track():
    ...
    try:
        ...
        if spotify_client:
            ...
            return jsonify({'success': False, 'error': 'Spotify not configured'}), 500
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def health_check():
    ...
    return jsonify({
        ...
        'gesture_system': 'ready' if gesture_system.client else 'not_configured',
...
def main():
    print("🤖 Qdrant-Based Face Recognition & Gesture Detection System")
    print("=" * 70)
    print("This application provides:")
    print("1. Face enrollment with liveness detection")
    print("2. Face authentication using Qdrant vector search")
    print("3. Gesture detection for Spotify control")
    print("4. Emotion detection using Roboflow")
    print("5. Unified API endpoints for all systems")
    print("\n🎮 Gesture Controls (when enabled):")
    print("- Show '0' (zero) to PAUSE")
    print("- Show '5' (five) to PLAY")
    print("\n🔐 Face Recognition:")
    print("- Uses Qdrant for fast vector similarity search")
    print("- Implements liveness detection")
    print("- Stores user metadata in MongoDB")
    print("\n😊 Emotion Detection:")
    print("- Uses Roboflow emotion-detection-cwq4g/1 model")
    print("- Detects emotions from multiple faces")
    print("- Independent of face authentication")
    print("\n🌐 Server endpoints:")
